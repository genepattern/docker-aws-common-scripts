...
do
    ...
    if [ ${GP_READONLY_BIND_MOUNTS,,} = "true"   ]; then
        ...
        if [ ${THE_DIR} == "$GP_JOB_METADATA_DIR"  ]; then
            ...
            RW_FLAG="  "
        elif [ "$THE_DIR" == "$GP_MODULE_DIR"  ]; then
            RW_FLAG="  "
        else 
            RW_FLAG=",readonly  "
        fi
    ...
    chmod -R a+rwx $GP_LOCAL_PREFIX${GP_MOUNT_POINT_ARRAY[i]}/..
    echo "==== OUTER CONTAINER DIR PERMISSIONS "
...
WALLTIME_UNIT="s"
export CONTAINER_NAME="GP_${GP_JOB_ID}"
...
then
   ...
   CONTAINER_ID="`docker run -d --name $CONTAINER_NAME --mount type=bind,src=$GP_LOCAL_PREFIX$GP_JOB_METADATA_DIR,dst=$GP_JOB_METADATA_DIR $MOUNT_STR -t $GP_JOB_DOCKER_IMAGE sleep ${GP_JOB_WALLTIME_SEC} `" 
else 
   CONTAINER_ID="`docker run -d --name $CONTAINER_NAME  --mount type=bind,src=$GP_LOCAL_PREFIX$GP_JOB_METADATA_DIR,dst=$GP_JOB_METADATA_DIR $MOUNT_STR -t $GP_JOB_DOCKER_IMAGE sleep ${GP_JOB_WALLTIME_SEC}s `" 
   exit_code=$?
fi
...
echo "--- copy in tasklib contents to $GP_MODULE_DIR"
...
# DEBUG LOOK AT FILE PERMISSIONS TO METADATA 
#docker exec  -t $CONTAINER_ID ls -alrt $GP_JOB_METADATA_DIR/..

# record memory and cpu every 30 s
bash /usr/local/bin/captureStats.sh 30s  $CONTAINER_ID  $GP_LOCAL_PREFIX$GP_JOB_METADATA_DIR/docker_stats.txt $GP_LOCAL_PREFIX$GP_JOB_METADATA_DIR/cap_stats.txt  &
